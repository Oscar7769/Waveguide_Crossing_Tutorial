...
* **factorization machine**要跟Crossing_Tutorial.ipynb或是Waveguide_Crossing_2.4x2.4.py放在同一資料夾才可以執行
* 執行時，在Terminal終端輸入```mpirun -n '核心數' python <py檔名>.py > <log檔名>.log 2>&1```(例如，mpirun -n 31 python Waveguide_Crossing.py > test.log 2>&1)
